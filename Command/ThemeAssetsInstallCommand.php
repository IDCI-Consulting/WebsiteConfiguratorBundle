...
namespace IDCI\Bundle\WebsiteConfiguratorBundle\Command;
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->getContainer()->get('filesystem')->mkdir($themesDir, 0777);
        ...
        foreach ($this->getContainer()->get('idci_website_configurator.website_registry')->getWebsites() as $website) {
            if (!is_dir($originDir = $website->getThemeAssetsPath())) {
                continue;
            }

            $targetDir = $themesDir.strtolower($website->getAlias());

            if (OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $message = sprintf("%s\n-> %s", $website->getAlias(), $targetDir);
            } else {
                $message = $website->getAlias();
            }

            try {
                $this->getContainer()->get('filesystem')->remove($targetDir);
    ...
    {
        if ($relative) {
            $originDir = $this->getContainer()->get('filesystem')->makePathRelative($originDir, realpath(dirname($targetDir)));
        }
        $this->getContainer()->get('filesystem')->symlink($originDir, $targetDir);
